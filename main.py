...
print(f"{Style.BRIGHT}{Fore.WHITE}Who Said It?")

while True:
  line = random.choice(lines)
  if len(list(line.values())[0]) < 25:
    continue

  print(list(line.values())[0])

  guess = input(f"\n{Fore.YELLOW}Guess: {Style.RESET_ALL}").upper()
  if guess == list(line.keys())[0]:
      input(f"{Fore.GREEN}Correct!{Style.RESET_ALL}")
  else:
      input(f"{Fore.RED}Wrong!{Style.RESET_ALL} This line was said by {Style.BRIGHT}{Fore.WHITE}{list(line.keys())[0]}{Style.RESET_ALL}.")
